...
public class ItemBuilder {
    ...
    public static ItemStack getFromJSON(Player player, String json) {
        ...
        try {
            ...
            if(object.has("material")){
                try{
                    material = Material.valueOf(object.get("material").getAsString());
                }catch (IllegalArgumentException e) {
                    player.sendMessage(ChatColor.RED+"Invalid material.");
                    return null;
                }
            }else{
                player.sendMessage(ChatColor.RED+"Need a Material.");
                return null;
            }
            ...
            if(object.has("name")){
                builder.displayname(ChatColor.translateAlternateColorCodes('&',object.get("displayName").getAsString()));
            }

            if(object.has("glow")) {
                assert object.get("glow").getAsBoolean();
                builder.glow();
            }

            if(object.has("enchants")){
                Map<Enchantment, Integer> enchantments = parseEnchantments(object,player);
                builder.enchant(enchantments);
            }

            return builder.build();
        } catch (ClassCastException e) {
            player.sendMessage(ChatColor.RED + "Unexpected object type!");
        } catch (Exception e) {
            player.sendMessage(ChatColor.RED + "Invalid JSON!");
            return null;
        }
        return null;
    }

    private static Map<Enchantment, Integer> parseEnchantments(JsonObject object,Player player) {
        JsonArray array = object.getAsJsonArray("enchants");
        Map<Enchantment, Integer> enchantmentIntegerMap = new HashMap<>();


        int index = 0;
        for(JsonElement element : array){
            if(element instanceof JsonObject enchantment){
                ...
                if(enchantment.has("lvl")){
                    level = enchantment.get("lvl").getAsInt();
                }

                if(!enchantment.has("ench")){
                    player.sendMessage(ChatColor.RED+"Invalid enchantment at index: "+index);
                    index++;
                    continue;
                }else{
                    try{
                        enchantment1 = Enchantment.getByName(enchantment.get("ench").getAsString().toUpperCase());
                    }catch (IllegalArgumentException e){
                        player.sendMessage(ChatColor.RED+"Invalid enchantment at index: "+index);
                        index++;
                        continue;
                ...
                enchantmentIntegerMap.put(enchantment1,level);
                index++;
                continue;
            }
